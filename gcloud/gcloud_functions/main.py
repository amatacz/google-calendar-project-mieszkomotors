...
def create_new_events(request, context=None):
    ...
    if not events_to_be_created:
        logger.log_text("No upcoming events - cannot proceed with events creation. Exiting...")
        ...
        if not existing_next_month_follow_up_events:
            logger.log_text("No existing events for following moth. Proceed with events creation.")
            ...
            try:
                ...
                logger.log_text(f"Error while creating events {e}")
            ...
            logger.log_text("Process finished successfully.")
